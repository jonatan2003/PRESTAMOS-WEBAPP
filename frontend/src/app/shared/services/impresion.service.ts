...
async imprimirFilaVentas(entidad: string, datos: any) {
  ...
  let boleta_factura ='          BOLETA DE VENTA ELECTRONICA ';
  ...
  const dni = `Dni: ${datos.dni}`;
  const empleado = `                  Empleado: ${datos.empleado} `;
  const descripcion = ` CANT      P.U                   Articulo                         `;
  ...
  const subtotal = `                                              SubTotal:   S/.${datos.total}`;
  // Contenido del encabezado
  const encabezado = [
    [empresa],
  ];
  ...
  articulos.forEach(detalle => {
    cuerpo.push([`     ${detalle.cantidad}` + "       "+ `S/.${detalle.precio_unitario}` + "     " + `${detalle.descripcion + " "+detalle.modelo + " " +detalle.marca}` ]);
  });
 ...
 autoTable(doc1, {
  ...
  else if (data.row.index === 14 || data.row.index === 19 || data.row.index === 21) {
    data.cell.styles.fontStyle = 'bold';
}
...
async imprimirFilaVentasF(entidad: string, datos: any) {
  ...
  const nro_serie = `                           ${datos.serie}`;
;
  let boleta_factura ='          FACTURA DE VENTA ELECTRONICA';
...
// IMPRESION DE LA VENTA FACTURA
async imprimirFilaVentasNC(entidad: string, datos: any) {
  const doc1 = new jsPDF({
    orientation: 'portrait',
    unit: 'in',
    format: [2.9, 5.5],
  ...
  const nro_serie = `                           ${datos.serie}`;
;
  let boleta_factura ='         NOTA DE CREDITO ELECTRONICA';
